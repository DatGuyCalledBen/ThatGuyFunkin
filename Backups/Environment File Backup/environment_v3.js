export function createScene(engine, canvas) {
    ...
    const transitionTime = 60 / (3 * BPM * QuantisationFactor); // Transition time in milliseconds
    ...
    if (!nextPosition || !nextPosition.alpha || !nextPosition.beta || !nextPosition.radius) {
        console.error('Next camera position is invalid.');
        return;
    }

    BABYLON.Animation.CreateAndStartAnimation('cameraTransitionAlpha', camera, 'alpha', 60, 120, camera.alpha, nextPosition.alpha, BABYLON.Animation.ANIMATIONLOOPMODE_CONSTANT);
    BABYLON.Animation.CreateAndStartAnimation('cameraTransitionBeta', camera, 'beta', 60, 120, camera.beta, nextPosition.beta, BABYLON.Animation.ANIMATIONLOOPMODE_CONSTANT);
    BABYLON.Animation.CreateAndStartAnimation('cameraTransitionRadius', camera, 'radius', 60, 120, camera.radius, nextPosition.radius, BABYLON.Animation.ANIMATIONLOOPMODE_CONSTANT, null, () => {
