export function createScene(engine, canvas) {
     ...
     function createNeighborhood(scene, rows, cols, spacing, parkSize, roadWidth)     {
        ...
        for (let i = -halfRows; i <= halfRows; i++) {
            for (let j = -halfCols; j <= halfCols; j++) {
                ...
                building.material.specularColor = new BABYLON.Color3(0, 0, 0);
    ...
    building1.material.specularColor = new BABYLON.Color3(0, 0, 0); // matte
    building2.material.specularColor = new BABYLON.Color3(0, 0, 0); // matte
    ...
    // Create ground
    const ground = BABYLON.MeshBuilder.CreateGround("ground", { width: 275, height: 275 }, scene);
    ...
    for (let i = 0; i < numberOfSteps; i++) {
        ...
        stepMaterial.specularColor = new BABYLON.Color3(0, 0, 0); // matte
    ...
    boxMaterial.specularColor = new BABYLON.Color3(1, 1, 1); // subtle shine
    boxMaterial.specularPower = 64;
    
    pyramidMaterial.specularColor = new BABYLON.Color3(0.2, 0.2, 0.2);
    pyramidMaterial.specularPower = 64;
    
    coneMaterial.specularColor = new BABYLON.Color3(1, 1, 1);
    coneMaterial.specularPower = 64;
    
    skyscraperMaterial.specularColor = new BABYLON.Color3(0.3, 0.3, 0.3); // more reflective
    skyscraperMaterial.specularPower = 128; // sharper highlights for glass effect
    
    // --- Ground ---
    groundMaterial.specularColor = new BABYLON.Color3(1, 1, 1); // matte
    ...
    let transitionTime = 1000*beatDuration*64; // Transition time in milliseconds
    ...
    function panCamera(camera) {
        ...
        animation.setKeys([
            ...
            { frame: 1000*beatDuration*32, value: camera.alpha - Math.PI/4}
        ]);
    
        camera.animations.push(animation);
        scene.beginAnimation(camera, 0, 1000*beatDuration*32, true);
    }
    ...
    function rotateCamera(camera) {
        ...
        animation.setKeys([
            ...
            { frame: 1000*beatDuration*32, value: camera.beta - Math.PI / 8}
        ]);
    ...
    function panCameraAlongPath(camera, path) {
        ...
        for (let i = 0; i < path.length; i++) {
            keys.push({ frame: i * (1000*beatDuration*32 / path.length), value: path[i] });
        }
    ...
    function applyZoomToFixedCamera(camera) {
        ...
        zoomAnimation.setKeys([
            ...
            { frame: 1000*beatDuration*32, value: camera.radius - 5 }
        ]);
    ...
    let transitionTime1 = beatDuration*16; // Transition time in seconds
    setInterval(() => {
    ...
    camera.layerMask = 0xFFFFFFFF; // Ensure the camera renders all layers except excluded ones
    ...
    let switchInterval = 1000*beatDuration*32; // Switch cameras every 10ish seconds
...
particleSystem.minEmitPower = 0.5*BPM;
particleSystem.maxEmitPower = 1.5*BPM;
...
// All cameras to include in SSR
const ssrCameras = [fixedCamera1, fixedCamera2, fixedCamera3, fixedCamera4, fixedCamera5, camera];

// Create SSR pipeline for all cameras
...
ssr.step = 15;
ssr.ignoreTransparentMeshes = true;
