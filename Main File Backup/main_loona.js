...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const spriteSheetData = [
        new SpriteSheet("down_sprite_sheet.png", 10, 1243, 617),
        new SpriteSheet("right_sprite_sheet.png", 10, 1409, 796),
        new SpriteSheet("up_sprite_sheet.png", 10, 1280, 993), 
        new SpriteSheet("left_sprite_sheet.png", 10, 1392, 878),
        new SpriteSheet("idle.png", 1, 1221, 932),
        new SpriteSheet("idle_sprite_sheet.png", 10, 1221, 932),
    ];
    ...
    sprite.position = new BABYLON.Vector3(-1.25, 2.5, 1.25);
    ...
    fetch('bassline-junkie-efan-bootleg-scdler [vocals].json')
        .then(response => {
            if (!response.ok) {
    ...
    function startAnimation() {
        try {
            ...
            const BPM = 139; // Example BPM (beats per minute), adjust as needed
            const quantizationFactor = 256; // Example quantization factor (4 for quarter notes), adjust as needed
    ...
    function createDefaultSpriteManager() {
        try {
            return new BABYLON.SpriteManager("defaultSpriteManager", "idle.png", 1, { width: 1221, height: 932 }, scene);
        } catch (error) {
